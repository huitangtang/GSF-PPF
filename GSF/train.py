...
def main():
    ...
    parser.add_argument('--one-score', action='store_true', 
                        help='whether to multiply confidence and synchronization as one score')
    parser.add_argument('--grad_sync_thres', type=float, default=None, 
                        help='gradient synchronization threshold in [0, 1] for filtering target samples')
    parser.add_argument('--grad_sync_start_epoch', type=int, default=1, 
                        help='at which epoch gradient synchronization starts')
    parser.add_argument('--build-workspace', action='store_true', 
                        help='whether to build workspace [args.confidence_threshold[0], args.confidence_threshold[-1]] for gradient synchronization filtering')
    parser.add_argument('--major-grad', default='all', type=str,
                        choices=['labeled', 'unlabeled', 'all'],
                        help='use majority gradient of each class on labeled or unlabeled data')
    parser.add_argument('--use-confident-samples', action='store_true', 
                        help='whether to use confident samples to compute majority gradient')
    parser.add_argument('--alpha', type=float, default=None, 
                        help='moving average coefficient')
    parser.add_argument('--normalize', action='store_true', 
                        help='whether to normalize instance gradients beforing summation')
    ...
    args.out = os.path.join(args.out, 'pseudo_label_method-' + args.pseudo_label_method + '_num_f-' + str(args.num_classifiers) + '_conf_thred-' + str(args.confidence_threshold) + '_grad_sync_thres-' + str(args.grad_sync_thres) + '_build_workspace-' + str(args.build_workspace) + '_useConfSamp-' + str(args.use_confident_samples) + '_alpha-' + str(args.alpha) + '_normalize-' + str(args.normalize) + '_lam_u-' + str(args.lambda_u) + '_mu-' + str(args.mu) + '_lr-' + str(args.lr) + '_arch-' + args.arch + '_classifier_type-' + args.classifier_type + '_seed-' + str(args.seed))
...
def compute_major_grad_each_class(grads, targets, major_grad_each_class, args, mask=None):
    ...
    if args.normalize:
        grads /= grads.norm(2, 1, True)
    tmp = (grads.unsqueeze(1) * targets_one_hot.unsqueeze(2)).sum(0)
    if args.alpha:
        alpha_vec = torch.zeros(args.num_classes, 1).cuda()
        alpha_vec[major_grad_each_class.norm(1, 1, True) != 0] = args.alpha
        alpha_vec[tmp.norm(1, 1, True) == 0] = 1
        major_grad_each_class = alpha_vec * major_grad_each_class + (1 - alpha_vec) * tmp
    else:
        major_grad_each_class += tmp
    
    del grads, tmp
    gc.collect()
    
    return major_grad_each_class
...
def train(args, labeled_trainloader, unlabeled_trainloader, test_loader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        major_grad_each_class = torch.zeros(args.num_classes, model['G'].module.channels if hasattr(model['G'], "module") else model['G'].channels).cuda()
        ...
        for batch_idx in range(args.eval_step):
            ...
            if args.grad_sync_thres is not None:
                Lu_w = F.cross_entropy(logits_u_w, targets_u, reduction='mean')
                loss_tmp = Lx + args.lambda_u * Lu_w
                grads = torch.autograd.grad(loss_tmp, [features], retain_graph=True)[0].detach().clone()
                grads = de_interleave(grads, 2*args.mu+1)
                grads_x, grads_u_w = grads[:batch_size], grads[batch_size:batch_size+inputs_u_w.size(0)]
                if args.major_grad == 'labeled':
                    major_grad_each_class = compute_major_grad_each_class(grads_x, targets_x, major_grad_each_class, args)
                elif args.major_grad == 'unlabeled':
                    major_grad_each_class = compute_major_grad_each_class(grads_u_w, targets_u, major_grad_each_class, args, mask_ct.unsqueeze(1) if args.use_confident_samples else None)
                elif args.major_grad == 'all':
                    major_grad_each_class = compute_major_grad_each_class(torch.cat([grads_x, grads_u_w], dim=0), torch.cat([targets_x, targets_u], dim=0), major_grad_each_class, args, mask=torch.cat([torch.ones(targets_x.size(0), 1).cuda(), mask_ct.unsqueeze(1)], dim=0) if args.use_confident_samples else None)
                
                if epoch + 1 >= args.grad_sync_start_epoch:
                    grad_sync = 0.5 * (1 + F.cosine_similarity(grads_u_w, major_grad_each_class[targets_u], dim=-1))
                    grad_sync[major_grad_each_class[targets_u].abs().sum(1) == 0] = 0
                    if args.one_score:
                        mask = (max_probs * grad_sync).ge(args.grad_sync_thres).float()
                    else:
                        mask_gst = grad_sync.ge(args.grad_sync_thres).float() # gradient synchronization filtering
                        mask = mask_ct * mask_gst
                    if args.build_workspace:
                        mask = mask if args.one_score else mask_gst
                        mask[mask_ct == 1] = 1
                        mask[max_probs <= args.confidence_threshold[0]] = 0
                        
                model['G'].zero_grad()
                model['F'].zero_grad()
                del grads, grads_x, grads_u_w
                gc.collect()
                torch.cuda.empty_cache()
            else:
                mask = mask_ct
